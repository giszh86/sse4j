...
package com.modestmaps.flex
{
    ...
    public class MapComponent extends UIComponent
	{
		 ...
		 */
		public function get map():com.modestmaps.TweenMap
		{
			return _map;
		}	
		...
		override protected function createChildren():void
		{
			...
			if (mapInitDirty && _map == null)
			{
				...
				_map = new TweenMap(unscaledWidth, unscaledHeight, _draggable, _mapProvider || DEFAULT_MAP_PROVIDER);
				_map.addEventListener(MouseEvent.DOUBLE_CLICK, _map.onDoubleClick);
		...
		override protected function commitProperties():void
		{
		    ...
		    if (_map!=null)
		    {
				...
				if(ovDirty){
					_map.addChild(new NavigatorWindow(_map));
					ovDirty = false;
				}
				
				if(navDirty){
					_map.addChild(new MapControls(_map,true,true));
					navDirty = false;
				}
		...
		protected var ovDirty:Boolean = false;
		protected var navDirty:Boolean = false;
		
		[Inspectable(category="Map")]
		public function set showOv(isShow:Boolean):void
		{
			ovDirty = isShow;
			invalidateProperties();
		}
		...
		[Inspectable(category="Map")]
		public function set showNav(isShow:Boolean):void
		{
			navDirty = isShow;
			invalidateProperties();
		}
