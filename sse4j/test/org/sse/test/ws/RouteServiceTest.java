...
public class RouteServiceTest {
	public static void main(String[] arg) throws Exception {
		...
		router.setStartPoint(gf
				.createPoint(new Coordinate(117.160778, 40.58228)));
		router.setEndPoint(gf.createPoint(new Coordinate(116.891613, 39.8071)));
		router.getViaPoints().add(
				gf.createPoint(new Coordinate(116.2753165, 39.9688405)));
		...
		for (int i = 0; i < result.getSegments().size(); i++) {
			if (i <= 1)
				System.out.println("start:" + i
						+ result.getSegments().get(i).getIds());
			...
			System.out.println(result.getSegments().get(i).getPoints().get(0)
					+ ", ");
			System.out.print(result.getSegments().get(i).getPoints().get(
					result.getSegments().get(i).getPoints().size() - 1)
					+ ", ");
		}
		for (RouteGuidance i : result.getGuidances()) {
			System.out.println(i.getName() + "--行使" + i.getLength() + "米--"
					+ i.getTurn() + "--用时" + i.getCost() + "秒" + i.getPoints());
		}
